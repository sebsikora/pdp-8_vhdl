...
architecture rtl of tty_state_gen is
	...
	begin
		...
		not_5:								NOT_gate port map (input => and_2_output, output => not_5_output);
		...
		ms_jk_ff_2:							ms_jk_ff port map ( j => ms_jk_ff_0_q, k => and_2_output, q => ms_jk_ff_2_q, not_q => ms_jk_ff_2_not_q, clk => SLOW_clk, not_reset => not_reset);
		
		TX <= nand_0_output;
		TX_FLAG <= ms_jk_ff_1_not_q;
		ser_clk <= and_4_output;
		SLOW_clk_not_clear <= not_5_output;
